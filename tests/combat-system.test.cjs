...
const { createCombatSystem, pickBuilderWorker, findNearbyResourceForWorker, resumeWorkerTask } = require('../js/combat.js');
...
const dmg = combat.calcDamage({ atk: 10, dmgType: 'magic' }, { armor: 0, armorType: 'heavy', magicResist: 0 });
assert.equal(dmg, 20, '应正确计算伤害倍率');

const target = { id: 1, owner: 'enemy', hp: 5, flash: 0, armor: 0, armorType: 'light', isBase: false };
combat.dealDamage({ id: 99, owner: 'player', atk: 10, dmgType: 'normal', splash: 0 }, target, 5);
...
assert.equal(workerToResume.state, 'moveToResource', '建造完成后应恢复之前的采集任务');
assert.equal(workerToResume.gatherTarget.x, 8, '建造完成后应恢复原来的资源目标');
